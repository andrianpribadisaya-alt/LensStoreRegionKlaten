  ...
  return (
    <div className="min-h-screen bg-[#020817] flex items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        {[["8%","15%","3s","0s"],["18%","78%","4s",".5s"],["75%","8%","3.5s",".8s"],["88%","85%","4s","1s"],["45%","92%","2.5s",".3s"],["60%","30%","3s","1.2s"]].map(([top,left,d,delay],i)=>(
          <div key={i} className="absolute w-0.5 h-0.5 bg-white rounded-full animate-pulse" style={{top,left,animationDuration:d,animationDelay:delay,opacity:0.4}} />
        ))}
      </div>
      ...
      <div className="w-full max-w-sm relative z-10">
        <div className="bg-[#0d1526] border border-white/[0.08] rounded-2xl p-7">
          ...
          <div className="text-center mb-6">
            <h1 className="text-xl font-medium text-slate-100">Log In</h1>
            <p className="text-sm text-slate-500 mt-1">Masuk ke akun LensOtp kamu</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <Link href="/forgot-password" className="text-xs text-sky-500 hover:text-sky-400">Forgotten Password?</Link>
            </div>
            ...
            <button type="submit" disabled={loading}
              ...
              {loading ? "Masuk..." : <>Log In <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7l5 5m0 0l-5 5m5-5H6"/></svg></>}
            </button>
          </form>
          ...
          <div className="flex items-center gap-3 my-5">
            ...
            <span className="text-xs text-slate-600">Or Log in with</span>
            <div className="flex-1 h-px bg-white/7" />
          </div>

          {/* Google */}
          <button onClick={handleGoogle}
            className="w-full py-2.5 bg-white/5 hover:bg-white/8 border border-white/10 hover:border-white/18 text-slate-300 text-sm rounded-xl flex items-center justify-center gap-2.5 transition mb-5">
          ...
          <p className="text-center text-xs text-slate-600">
            ...
            <Link href="/register" className="text-sky-500 hover:text-sky-400">Create Account</Link>
          </p>
        </div>
      </div>
    </div>
