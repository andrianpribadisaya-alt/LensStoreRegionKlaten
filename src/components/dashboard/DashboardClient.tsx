...
const TIER_BG: Record<string, string> = {
  ...
  admin: "from-red-500/20 to-red-800/10 border-red-500/20",
};

function getTierProgress(role: string, totalOrder: number, limits: TierLimits) {
  if (role === "platinum" || role === "admin") return { next: null, percent: 100, current: totalOrder, target: 0 };
  ...
  return { next: nextTiers[role], percent, current: totalOrder, target };
}

const PLATFORM_ICONS: Record<string, string> = {
  whatsapp: "💬", telegram: "✈️", instagram: "📸", tiktok: "🎵",
  facebook: "👥", google: "🔍", gmail: "📧", twitter: "🐦",
  discord: "🎮", openai: "🤖", chatgpt: "🤖", apple: "🍎",
  microsoft: "🪟", shopee: "🛒", tokopedia: "🟢", gojek: "🚗",
};
...
export default function DashboardClient({ user, services, tierLimits, announcement }: Props) {
  ...
  return (
    <div className="space-y-4">
      {/* Ticker Notifikasi dari admin */}
      {announcement.enabled && announcement.message && (
        <div className="bg-sky-500/5 border border-sky-500/15 rounded-xl px-3 py-2 overflow-hidden flex items-center gap-2">
          {announcement.title && (
          ...
          <div className="flex-1 overflow-hidden">
            <span className="text-xs text-slate-400 whitespace-nowrap inline-block animate-[marquee_15s_linear_infinite]">
      ...
      <div>
        ...
        <p className="text-xs text-slate-500 mt-0.5">Berikut ringkasan akun kamu hari ini</p>
      </div>

      {/* Balance Card */}
      <div className="rounded-2xl bg-gradient-to-r from-sky-500 to-indigo-600 p-5 text-white relative overflow-hidden">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-2xl bg-slate-900 border border-white/[0.06] p-4">
          <div className="w-8 h-8 rounded-xl bg-sky-500/10 flex items-center justify-center mb-3">
      ...
      <div className={`rounded-2xl bg-gradient-to-r border p-4 ${TIER_BG[role] ?? TIER_BG.bronze}`}>
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className={`text-xl font-semibold capitalize mt-0.5 ${TIER_COLORS[role]}`}>{role}</p>
          </div>
          {tierProgress.next && (
        ...
        {tierProgress.next ? (
          <>
            <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-sky-400 to-indigo-500 rounded-full transition-all" style={{ width: `${tierProgress.percent}%` }}/>
            </div>
      ...
      <div>
        ...
        <div className="relative mb-3">
          ...
          <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Cari platform..."
            className="w-full pl-9 pr-4 py-2.5 bg-slate-900 border border-white/[0.06] rounded-xl text-sm text-slate-300 placeholder-slate-600 focus:outline-none focus:border-sky-500/40 transition"/>
        </div>
        <div className="space-y-2">
          {filteredServices.length === 0 ? (
            <div className="text-center py-8 text-slate-600 text-sm">
              {services.length === 0 ? "Belum ada platform. Admin perlu sync provider dulu." : "Platform tidak ditemukan."}
            </div>
          ) : filteredServices.map(service => (
            <div key={service._id} className="flex items-center justify-between px-4 py-3 bg-slate-900 border border-white/[0.06] rounded-xl hover:border-white/12 transition cursor-pointer">
